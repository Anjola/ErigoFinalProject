...
public class Server {
	...
	//stores all messages with a uniqueMessageID with corresponding JSON pair
	protected static final Map<String, String> Messages = Collections.synchronizedMap(new HashMap<String, String>());
	
	//clean up task 
	private final ScheduledExecutorService scheduler = Executors.newScheduledThreadPool(1);
	//for unique id generation. synchronized int
	protected static final AtomicLong  clientID = new AtomicLong();
	//for unique id generation for storing unique messages
	protected static final AtomicLong MessageID = new AtomicLong();
	// constructor
	Server(int port) {
		this.port = port;
	}
